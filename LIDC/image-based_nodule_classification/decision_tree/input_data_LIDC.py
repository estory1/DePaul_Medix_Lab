...
def read_data_sets(images_root_dir_path, images_root_dir_path_match_str, images_file_glob, master_join4_file_path, patient_dicom_root_dir_path, patient_dicom_file_glob, fake_data=False, one_hot=False):

  images, labels = read_images_and_labels(images_root_dir_path, images_root_dir_path_match_str, images_file_glob, master_join4_file_path, patient_dicom_root_dir_path, patient_dicom_file_glob)
...
def read_images_and_labels(images_root_dir_path, images_root_dir_path_match_str, images_file_glob, master_join4_file_path, patient_dicom_root_dir_path, patient_dicom_file_glob="*.csv"):
  """
  ...
  allFilePaths = list([f for f in find_files(images_root_dir_path, images_file_glob) if images_root_dir_path_match_str in f])
  ...
  if len(allFilePaths) == 0:
    ...
    def multiProcAllFiles(image_data, malignancy_series, dicom_files, master_join4, allFilePaths, lock, output):
      ...
      for filePath in allFilePaths:
        ...
        XmlStudyNode = fpArr[-6]
        StudyInstanceUID = fpArr[-5]
        SeriesInstanceUID = fpArr[-4]
        fpFilename = fpArr[-1]
        esprint(fpFilename + " : " + "'" + fpFilename.split('-')[1] + "'")
        roi_id = int(re.search("roi_\d+", fpFilename).group(0).split('_')[1])
        rs_id = int(re.search("rs_\d+", fpFilename).group(0).split('_')[1])
        orig_dicom_filename = fpFilename.split('-')[0] + ".dcm"

        try:
          # 2) query DICOM extract file for the imageSOP_UID.
          # esprint("*** 1 *** - " + str(fpArr) + " - " + str(XmlStudyNode))
