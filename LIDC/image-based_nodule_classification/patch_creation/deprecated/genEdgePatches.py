...
# Example execution:
#
# ./genEdgePatches.py ../../LIDC_Complete_20141106/Extracts/master_join4.csv ../../LIDC_Complete_20141106/LIDC-IDRI/LIDC-IDRI-0001/1.3.6.1.4.1.14519.5.2.1.6279.6001.298806137288633453246975630178/1.3.6.1.4.1.14519.5.2.1.6279.6001.179049373636438705059720603192/ 000071.dcm
...
import patchAnalysis


master_join4_file_path = sys.argv[1]
images_root_path = sys.argv[2]
images_file_glob = sys.argv[3]
...
def read_images_and_labels(images_root_dir_path, images_file_glob):
  ...
  if len(allFilePaths) == 0:
    ...
    with open(master_join4_file_path, 'rb') as f:
      master_join4 = pd.read_csv(f, delimiter=",", header=1, usecols=["imageSOP_UID", "roi_Id", "readingSession_Id", "malignancy", "EdgeMap"])
      master_join4 = master_join4.fillna(-1)
    print master_join4.columns.values
    imageSOP_UID_all = np.array(master_join4["imageSOP_UID"])
    edge_maps_all = np.array(master_join4["EdgeMap"])
    ...
    def multiProcAllFiles(image_data, allFilePaths, lock, output):
      ...
      for filePath in allFilePaths:
        ...
        # src: https://pyscience.wordpress.com/2014/09/08/dicom-in-python-importing-medical-image-data-into-numpy-with-pydicom-and-vtk/
        d = dicom.read_file(filePath)
        dims = (int(d.Rows), int(d.Columns))
        width = dims[0]
        height = dims[1]

        # get the identifying/matching values
        imageSOP_UID = d.SOPInstanceUID
        # roi_Id = d.roi_Id
        # readingSession_Id = d.readingSession_Id
        imageSOP_UID_match_rslts = np.array(imageSOP_UID_all == imageSOP_UID)
        imageSOP_UID_match_idxs = np.argwhere(imageSOP_UID_match_rslts == True)
        edge_maps_matches = np.array(edge_maps_all[imageSOP_UID_match_idxs])
        print edge_maps_matches
        edge_maps_numeric = []
        for edgeStrArr in edge_maps_matches:
          for pairStr in edgeStrArr[0].split('|'):
            mNumeric = []
            for v in pairStr.split(';'):
              mNumeric.append(int(v))
            edge_maps_numeric.append(np.array(mNumeric))
        edge_maps_numeric = np.array(edge_maps_numeric)

        edge_map_numeric_sortXAsc = np.sort(edge_maps_numeric.view('i8,i8'), order=['f0'], axis=0).view(np.int)
        edge_map_numeric_sortYAsc = np.sort(edge_maps_numeric.view('i8,i8'), order=['f1'], axis=0).view(np.int)

        # print [ imageSOP_UID, edge_maps_numeric ]

        # print d
        # return

        # n = # of non-overlapping patches along x or y dimension to create.
        n = 64
        wUnit = (width / n)
        hUnit = (height / n)
        pfnPrefix = (filePath.split(".dcm"))[0]
        # For each row in the set of edge patches to create:
        for i in range(0,n):
          # For each col in the set of edge patches to create:
          for j in range(0,n):
            # Compute the image pixel subset indexes.
            pxs = wUnit * j
            pys = hUnit * i
            pxe = wUnit * (j + 1)
            pye = hUnit * (i + 1)
            #   Create the patch file name.
            pfnPostfix = "-" + str(pxs) + "," + str(pys) + "_to_" + str(pxe) + "," + str(pye)
            pfn = pfnPrefix + pfnPostfix + ".tiff"
            # print pfn
            #   Create the edge patch image.
            p = d.pixel_array[pxs:pxe,pys:pye]
            p = mat2gray(p)
            
            # write the image.
            outFileName = "delme" + pfnPostfix + ".tiff"
            # if (np.min(p) < 1):
            #   print outFileName + ": " + str(np.shape(p)) + ": " + str(p)
            scipy.misc.imsave(outFileName, p)

            # # write a new, tiny DICOM file. Src: https://pydicom.readthedocs.org/en/stable/working_with_pixel_data.html
            # d.PixelData = p.tostring()
            # d.Rows = (width / pn)
            # d.Cols = (height / pn)
            # d.save_as("delme" + pfnPostfix + ".dcm")

            # Create binary of patch.
            patchBin = patchAnalysis.computeBinary(pxs, pys, pxe, pye, wUnit, hUnit, edge_map_numeric_sortXAsc, edge_map_numeric_sortYAsc)
            # print np.mean(patchBin)

            #   Create the edge patch image metadata structure.
...
if __name__ == "__main__":
  esprint([master_join4_file_path, images_root_path, images_file_glob])
  image_data = read_images_and_labels(images_root_path, images_file_glob)
