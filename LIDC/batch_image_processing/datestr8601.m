% Src: http://www.mathworks.com/matlabcentral/fileexchange/34095-date-vector-number-to-iso-8601-date-string/content/datestr8601.m
...
%  Str = datestr8601(DVN,Tok1,Tok2,...)
%  [Str1,Str2,...] = datestr8601(DVN,Tok1,Tok2,...)
%  [...] = datestr8601([],...)
%
% Easy conversion of a Date Vector or Serial Date Number to Date String/s,
% with a date format controlled by (optional) input token/s. Each token defines
% a string with either an ISO 8601 timestamp or a single date/time value: if
% there are more input tokens than outputs, then the remaining strings are
% concatenated together (with a space character between each string). Calling
% the function with multiple tokens is faster than multiple function calls.
%
...
% files that can be sorted ASCIIbetically into chronological order!
%
...
%
% See also DATENUM8601 DATEROUND CLOCK NOW DATESTR DATENUM DATEVEC NATSORT NATSORTROWS NATSORTFILES
%
% ### Examples ###
%
...
%  D3 = 'Fri'
%
% datestr8601(clock,'DDDD','ddd','mmmm','yyyy')
...
for m = 1:numel(DfAr)
    ...
    switch Tok
        ...
        case {'W','WW','WWW'} % too confusing: 'w','ww','www'
            % week of the year
...
if nargin-1 > nargout
    idx = max(1,nargout);
    varargout{idx} = varargout(idx:end);
    varargout{idx}(2,:) = {' '};
    varargout{idx} = [varargout{idx}{1:end-1}];
end
%
end
%----------------------------------------------------------------------END:datestr8601
